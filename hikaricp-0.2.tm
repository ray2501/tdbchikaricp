...
#	Class definitions and Tcl-level methods for the tdbc::hikaricp bridge.
#
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
#
# tdbc::hikaricp::connection --
#
#	Class representing a connection to a hikaricp database.
#
#-------------------------------------------------------------------------------

::oo::class create ::tdbc::hikaricp::connection {

    superclass ::tdbc::connection

    variable ConnectionI DatabaseMetaDataI
    variable DataSourceI
    variable isolation
    variable readonly
    ...
    constructor {configFile {datasource 0} args} {
        ...
        if {[llength $args] % 2 != 0} {
            ...
            return -code error \
            -errorcode {TDBC GENERAL_ERROR HY000 HikariCP WRONGNUMARGS} \
    ...
    forward statementCreate ::tdbc::hikaricp::statement create
    ...
    method configure args {
        if {[llength $args] == 0} {
            ...
            switch -exact -- $option {
                ...
                default {
                    return -code error \
                    -errorcode [list TDBC GENERAL_ERROR HY000 HikariCP \
                            BADOPTION $option] \
...
#------------------------------------------------------------------------------
#
# tdbc::hikaricp::statement --
#
#	The class 'tdbc::hikaricp::statement' models one statement against a
#       database accessed through a jdbc connection
#
#------------------------------------------------------------------------------

::oo::class create ::tdbc::hikaricp::statement {
    ...
    forward resultSetCreate ::tdbc::hikaricp::resultset create
    ...
    method paramtype args {
        ...
        if { [catch  {set value [dict get $Params $parameter]}] } {
            set cmd [lrange [info level 0] 0 end-[llength $args]]
            return -code error \
            -errorcode {TDBC GENERAL_ERROR HY000 hikaricp BADOPTION} \
            "wrong param...\""
        }
...
#------------------------------------------------------------------------------
#
# tdbc::hikaricp::resultset --
#
#	The class 'tdbc::hikaricp::resultset' models the result set that is
#	produced by executing a statement against a jdbc database.
#
#------------------------------------------------------------------------------

::oo::class create ::tdbc::hikaricp::resultset {
                ...
                foreach mykey $keylist {
                    ...
                    if {[info exists ::$mykey] == 1} {
                        ...
                        switch -exact -- ${-sqltypes} {
                            ...
                            clob {
                                # only work for type 4 HikariCP driver
